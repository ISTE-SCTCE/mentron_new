...
export default async function FolderNotesPage({
    params,
}: {
    params: Promise<{ year: string; dept: string; sem: string; subject: string; folderId: string }>
}) {
    const { year, dept, sem, subject, folderId } = await params
    ...
    const subjectUrl = `/notes/year/${yearNum}/dept/${deptKey}/${semKey}/${encodeURIComponent(subjectName)}`
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <p className={`text-[10px] font-black tracking-[0.3em] uppercase ${style.accent}`}>
                                    {yearMeta.label} · {semKey} · {deptKey} · {subjectName}
                                </p>
