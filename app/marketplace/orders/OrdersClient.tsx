...
interface Order {
  ...
  created_at: string
  marketplace_listings?: { id: string; title: string; images: string[] } | null
...
const statusColor = (s: string) => {
  ...
  if (normalized === 'completed' || normalized === 'delivered' || normalized === 'payment_confirmed') {
...
function OrderCard({ order, label }: { order: Order; label: 'Seller' | 'Buyer' }) {
  ...
  const title =
    order.marketplace_listings?.title || order.marketplace_items?.title || 'Marketplace Item'
  const imageUrl =
    order.marketplace_listings?.images?.[0] || order.marketplace_items?.image_url || ''
  const priceVal = order.amount ?? order.price ?? 0
  ...
  return (
    ...
    >
      ...
      {imageUrl ? (
        ...
        <div className="w-14 h-14 rounded-xl bg-white/5 flex items-center justify-center shrink-0 text-xl">
          📦
        </div>
      )}
      ...
      <div className="flex-1 min-w-0">
        ...
        {order.utr_number && (
          <p className="text-[10px] text-gray-500 mt-1 font-mono">UTR: {order.utr_number}</p>
        )}
        {order.note && (
      ...
      <div className="flex flex-col items-end gap-2 shrink-0">
        <p className="font-black text-white text-sm">₹{Number(priceVal).toLocaleString('en-IN')}</p>
        <span
          className={`text-[9px] font-black uppercase tracking-widest px-2 py-1 rounded-lg border ${statusColor(
            currentStatus
          )}`}
        >
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12">
      ...
      <section>
        <motion.div
          ...
          className="mb-6"
        >
          <p className="text-[10px] font-black tracking-widest text-cyan-400 uppercase mb-1">
            What I Bought
          </p>
          <h2 className="text-xl sm:text-2xl font-black text-white">Purchases</h2>
          <p className="text-gray-500 text-xs mt-1">
            {myPurchases?.length ?? 0} order{myPurchases?.length !== 1 ? 's' : ''}
          </p>
        </motion.div>
        ...
        {!myPurchases?.length ? (
          ...
          >
            ...
            <Link
              href="/marketplace"
              className="mt-3 inline-block text-cyan-400 text-xs font-black uppercase tracking-widest hover:text-cyan-300 transition-colors"
            >
              Browse Items →
            </Link>
          </motion.div>
      ...
      <section>
        <motion.div
          ...
          className="mb-6"
        >
          <p className="text-[10px] font-black tracking-widest text-purple-400 uppercase mb-1">
            What I Sold
          </p>
          <h2 className="text-xl sm:text-2xl font-black text-white">Orders Received</h2>
          <p className="text-gray-500 text-xs mt-1">
            {ordersReceived?.length ?? 0} order{ordersReceived?.length !== 1 ? 's' : ''}
          </p>
        </motion.div>
        ...
        {!ordersReceived?.length ? (
          ...
          >
            ...
            <Link
              ...
              className="mt-3 inline-block text-purple-400 text-xs font-black uppercase tracking-widest hover:text-purple-300 transition-colors"
            >
              List an Item →
            </Link>
          </motion.div>
